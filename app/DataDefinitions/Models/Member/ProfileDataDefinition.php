...
class ProfileDataDefinition extends DataDefinition
{
    ...
    public function __construct()
    {
        ...
        $this->fields = [
            'tab1' => [
                ...
                'fields' => [
                    ...
                    'accepts_emails' => (auth('member')->user()?->email) ? [
        ...
        // ─────────────────────────────────────────────────────────────────
        // Switch Account tab for device linking
        // Only shown for anonymous members so they can link to their
        // existing account from a new device
        // ─────────────────────────────────────────────────────────────────
        $isAnonymous = auth('member')->user()?->isAnonymous() ?? false;

        if ($isAnonymous) {
            // Anonymous members: Insert Switch Account as FIRST tab
            // Shift existing tabs and add switch account at the beginning
            $existingFields = $this->fields;
            $this->fields = [
                'tab1' => [
                    'title' => trans('common.switch_account.tab_title'),
                    'type' => 'custom',
                    'view' => 'member.account.switch-account',
                ],
            ];
            // Re-add existing tabs with incremented keys (tab2, tab3, etc.)
            $tabIndex = 2;
            foreach ($existingFields as $tab) {
                $this->fields['tab' . $tabIndex] = $tab;
                $tabIndex++;
            }
        }

        // ─────────────────────────────────────────────────────────────────
        // Privacy & Data tab is only shown to members with an email address
        // Anonymous members can't use password reset, data export, etc.
        // ─────────────────────────────────────────────────────────────────
        if (auth('member')->user()?->email) {
            $this->fields['tab3'] = [
                'title' => trans('common.privacy_and_data'),
                'type' => 'custom',
                'view' => 'member.account.privacy-data',
            ];
        }
