...
class ReferralCompletedReferee extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable): MailMessage
    {
        ...
        // Set locale for translations and URLs based on the referee's preference
        set_url_locale_for_user($notifiable);

        return (new MailMessage)
            ->theme('platform')
            ->from($mailFromAddress, $mailFromName)
            ->subject(trans('email.referral_referee_subject', ['club' => $this->referral->club->name]))
            ->view('emails.referral.referee-completed', [
    ...
    public function toArray($notifiable): array
    {
        return [
            ...
            'points' => $this->points,
            'event' => 'referral_completed_referee',
        ];
    }
}
