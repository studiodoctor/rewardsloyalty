...
@section('page_title', trans('common.history') . config('default.page_title_delimiter') . ($voucher ? $voucher->code : '') . config('default.page_title_delimiter') . ($member ? $member->name : '') . config('default.page_title_delimiter') . config('default.app_name'))
...
<div class="min-h-screen bg-gradient-to-br from-stone-50 via-white to-stone-100 dark:from-secondary-950 dark:via-secondary-900 dark:to-secondary-950">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        <div class="absolute top-1/4 left-1/3 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl animate-pulse"></div>
        <div class="absolute bottom-1/4 right-1/3 w-72 h-72 bg-violet-500/10 rounded-full blur-3xl animate-pulse" style="animation-delay: 2s;"></div>
    </div>
        ...
        @if($member && $voucher)
            ...
            <x-ui.page-header
                ...
                :breadcrumbs="[
                    ...
                    ['url' => route('member.voucher', ['voucher_id' => $voucher->id]), 'text' => trans('common.view_voucher'), 'target' => '_blank']
                ]"
                compact
            />
            ...
            {{-- Voucher Display --}}
            <div class="mb-6">
                <x-member.voucher-card :voucher="$voucher" :links="false" />
            </div>

            {{-- Quick Actions --}}
            <div class="flex gap-3 mb-6">
                <a href="{{ route('member.voucher', ['voucher_id' => $voucher->id]) }}" 
                   target="_blank"
                   class="flex-1 flex items-center justify-center gap-2 px-4 py-3 
                    ...
                    {{ trans('common.view_voucher') }}
                </a>
                
                {{-- Delete Last Redemption --}}
                <div x-data="{ confirming: false }" class="flex-1">
                    ...
                    <button @click="confirming = true" 
                        ...
                        {{ trans('common.delete_last_redemption') }}
                    </button>
                    ...
                    <div x-show="confirming"
                        ...
                        <a href="{{ route('partner.delete.last.voucher.redemption', ['member_identifier' => $member->unique_identifier, 'voucher_id' => $voucher->id]) }}"
                           class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 
            ...
            <div class="space-y-6">
                <div>
                    <x-member.member-card :member="$member" :club="$voucher->club" :show-tier="true" />
                </div>
                ...
                <div>
                    <div class="bg-white dark:bg-secondary-900 rounded-xl 
                        ...
                        <div class="px-6 py-4 border-b border-stone-200 dark:border-secondary-800 
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-secondary-500 dark:text-secondary-400">{{ trans('common.voucher_redemptions') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- History Content --}}
                        <div class="p-6">
                            <x-member.voucher-history :voucher="$voucher" :member="$member" :show-notes="true" :show-staff="true" />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div>
                        <div class="bg-rose-50 dark:bg-rose-950/30 
                            ...
                            <div class="w-16 h-16 mx-auto rounded-xl 
                                        bg-rose-100 dark:bg-rose-900/50 
                                        flex items-center justify-center mb-4">
                                <x-ui.icon icon="ticket" class="w-8 h-8 text-rose-600 dark:text-rose-400" />
                            </div>
                            <h3 class="text-lg font-semibold text-rose-900 dark:text-rose-300">{{ trans('common.voucher_not_found') }}</h3>
                        </div>
                    </div>
                @endif
            </div>
