{{--
  ...
  Verifies email after registration via 6-digit code.
--}}

@extends('member.layouts.default', ['robots' => false, 'authPage' => true])

@section('page_title', trans('otp.registration_verify_title') . config('default.page_title_delimiter') . config('default.app_name'))
...
<section class="min-h-screen bg-white dark:bg-secondary-950">
    <div class="w-full min-h-screen grid lg:grid-cols-2">
        ...
        <div
            ...
            <div class="w-full max-w-[440px] space-y-8 animate-fade-in-up">
                ...
                <div class="text-center space-y-3">
                    {{-- Email Icon --}}
                    <div class="mx-auto w-16 h-16 rounded-2xl bg-gradient-to-br from-green-500/10 to-emerald-500/10 dark:from-green-500/20 dark:to-emerald-500/20 flex items-center justify-center mb-4">
                        <x-ui.icon icon="mail-check" class="w-8 h-8 text-green-600 dark:text-green-400" />
                    </div>

                    <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-secondary-900 dark:text-white">
                        {{ trans('otp.registration_verify_title') }}
                    </h2>
                    <p class="text-secondary-500 dark:text-secondary-400">
                        {{ trans('otp.registration_verify_subtitle') }}
                        <br>
                ...
                {{-- OTP Form --}}
                <x-forms.form-open id="otp-form" class="space-y-6" :action="route('member.register.otp.verify.post')" method="POST" />
                    ...
                    {{-- Resend Code --}}
                    <div 
                        x-data="resendCooldown({ initialCooldown: {{ $cooldown ?? 0 }} })"
                        class="text-center"
                    >
                    ...
                    <div class="pt-4 text-center">
                        <a href="{{ route('member.register') }}" class="text-sm font-medium text-secondary-500 hover:text-secondary-700 dark:text-secondary-400 dark:hover:text-secondary-200 transition-colors">
                            ← {{ trans('otp.registration_start_over') }}
                        </a>
                    </div>
                <x-forms.form-close />
            </div>
        ...
        <div class="hidden lg:flex relative bg-secondary-900 items-center justify-center overflow-hidden">
            <div
                class="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-green-600 via-emerald-700 to-teal-900">
            </div>

            {{-- Abstract Shapes --}}
            <div
                class="absolute top-0 right-0 -translate-y-1/4 translate-x-1/4 w-[500px] h-[500px] bg-emerald-500/20 rounded-full blur-[100px] animate-pulse-slow">
            </div>
            <div class="absolute bottom-0 left-0 translate-y-1/4 -translate-x-1/4 w-[500px] h-[500px] bg-teal-500/20 rounded-full blur-[100px] animate-pulse-slow"
            ...
            <div class="relative z-10 max-w-lg px-12 text-center animate-fade-in-up delay-200">
                ...
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/10 mb-8">
                    <x-ui.icon icon="sparkles" class="w-5 h-5 text-yellow-400" />
                    <span class="text-sm font-medium text-white/90">{{ trans('otp.registration_almost_there') }}</span>
                </div>

                <h1 class="text-3xl md:text-4xl font-bold text-white mb-6 tracking-tight leading-tight drop-shadow-lg">
                    {{ trans('otp.registration_verify_panel_title') }}
                </h1>

                <p class="text-lg text-secondary-300 font-light leading-relaxed">
                    {{ trans('otp.registration_verify_panel_text') }}
                </p>
                ...
                <div class="mt-10 space-y-3 text-left">
                    <div class="flex items-start gap-3 text-secondary-300/80">
                        ...
                        <span class="text-sm">{{ trans('otp.registration_benefit_1') }}</span>
                    </div>
                    <div class="flex items-start gap-3 text-secondary-300/80">
                        <x-ui.icon icon="check-circle" class="w-5 h-5 flex-shrink-0 mt-0.5 text-green-400" />
                        <span class="text-sm">{{ trans('otp.registration_benefit_2') }}</span>
                    </div>
                    <div class="flex items-start gap-3 text-secondary-300/80">
                        <x-ui.icon icon="check-circle" class="w-5 h-5 flex-shrink-0 mt-0.5 text-green-400" />
                        <span class="text-sm">{{ trans('otp.registration_benefit_3') }}</span>
                    </div>
                </div>
            </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('resendCooldown', (config = {}) => ({
        ...
        async resend() {
            ...
            try {
                const response = await fetch('{{ route('member.register.otp.resend') }}', {
                    method: 'POST',
                    headers: {
