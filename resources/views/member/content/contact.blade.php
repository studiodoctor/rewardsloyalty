...
@section('page_title', ($meta['title'] ?? trans('common.contact')) . config('default.page_title_delimiter') . config('default.app_name'))
...
<div class="min-h-screen">
    ...
    <header class="relative overflow-hidden">
        {{-- Gradient background --}}
        <div class="absolute inset-0 bg-gradient-to-br from-emerald-50 via-white to-teal-50/50 dark:from-secondary-900 dark:via-secondary-900 dark:to-emerald-950/20"></div>
        <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-emerald-200/30 via-transparent to-transparent dark:from-emerald-500/10 animate-pulse" style="animation-duration: 5s;"></div>
        
        {{-- Floating elements --}}
        <div class="absolute top-10 right-20 w-64 h-64 bg-emerald-400/10 dark:bg-emerald-500/5 rounded-full blur-3xl animate-float"></div>
        <div class="absolute bottom-20 left-10 w-80 h-80 bg-teal-300/10 dark:bg-teal-600/5 rounded-full blur-3xl animate-float-delayed"></div>
        
        <div class="relative max-w-5xl mx-auto px-6 py-16 md:py-24">
            {{-- Back Navigation --}}
            <a href="{{ route('member.index') }}" 
                class="inline-flex items-center gap-2 text-sm font-medium text-secondary-600 dark:text-secondary-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors mb-8 group animate-fade-in">
            ...
            <div class="text-center">
                {{-- Icon --}}
                <div class="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-xl shadow-emerald-500/25 mb-8 animate-fade-in-up">
                    <x-ui.icon icon="{{ $meta['icon'] ?? 'mail' }}" class="w-10 h-10" />
                </div>
                ...
                <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-6 animate-fade-in-up" style="animation-delay: 100ms;">
                    <span class="bg-gradient-to-r from-secondary-900 via-secondary-700 to-secondary-900 dark:from-white dark:via-secondary-200 dark:to-white bg-clip-text text-transparent">
                        {{ $meta['title'] ?? trans('common.contact') }}
                    </span>
                </h1>
