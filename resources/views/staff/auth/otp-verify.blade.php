{{--
  ...
  Staff OTP Verification Page
  Premium design inspired by Linear, Revolut, and Stripe.
--}}

@extends('staff.layouts.default', ['robots' => false, 'authPage' => true])

@section('page_title', trans('otp.step3_title') . config('default.page_title_delimiter') . config('default.app_name'))
...
<section class="min-h-screen bg-white dark:bg-secondary-950">
    <div class="w-full min-h-screen grid lg:grid-cols-2">
        ...
        <div
            ...
            <div class="w-full max-w-[440px] space-y-8 animate-fade-in-up">
                ...
                <div class="text-center space-y-3">
                    {{-- Email Icon --}}
                    <div class="mx-auto w-16 h-16 rounded-2xl bg-gradient-to-br from-accent-500/10 to-emerald-500/10 dark:from-accent-500/20 dark:to-emerald-500/20 flex items-center justify-center mb-4">
                        <x-ui.icon icon="mail-check" class="w-8 h-8 text-accent-600 dark:text-accent-400" />
                    </div>
                ...
                {{-- OTP Form --}}
                <x-forms.form-open id="otp-form" class="space-y-6" :action="route('staff.login.otp.verify.post')" method="POST" />
                    ...
                    >
                        ...
                        <button
                            ...
                            :class="{
                                'text-accent-600 hover:text-accent-500 dark:text-accent-400 dark:hover:text-accent-300': cooldown === 0 && !loading,
                    ...
                    <div class="pt-4 text-center">
                        <a href="{{ route('staff.login') }}" class="text-sm font-medium text-secondary-500 hover:text-secondary-700 dark:text-secondary-400 dark:hover:text-secondary-200 transition-colors">
                            ← {{ trans('otp.step3_try_another') }}
                        </a>
                    </div>
                <x-forms.form-close />
            </div>
        ...
        <div class="hidden lg:flex relative bg-secondary-900 items-center justify-center overflow-hidden">
            <div
                class="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-accent-600 via-accent-700 to-accent-900">
            </div>
            ...
            <div class="relative z-10 max-w-lg px-12 text-center animate-fade-in-up delay-200">
                ...
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/10 mb-8">
                    ...
                    <span class="text-sm font-medium text-white/90">{{ trans('otp.staff_portal') }}</span>
                </div>
                ...
                <p class="text-lg text-secondary-300 font-light leading-relaxed">
                    {{ trans('otp.step3_panel_description') }}
                </p>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('resendCooldown', (config = {}) => ({
        ...
        async resend() {
            ...
            try {
                const response = await fetch('{{ route('staff.login.otp.resend') }}', {
                    method: 'POST',
                    headers: {
