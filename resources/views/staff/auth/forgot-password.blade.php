...
<section class="min-h-screen bg-white dark:bg-secondary-950">
    <div class="w-full min-h-screen grid lg:grid-cols-2">
        ...
        <div
            ...
            <div class="w-full max-w-[440px] space-y-8 animate-fade-in-up">
                ...
                @if (!Session::has('success'))
                    <x-forms.form-open class="space-y-6" :action="route('staff.forgot_password.post')" method="POST" />
                    <div class="space-y-5">
        ...
        <div class="hidden lg:flex relative bg-secondary-900 items-center justify-center overflow-hidden">
            <div
                class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-accent-600 via-accent-700 to-accent-900">
            </div>

            {{-- Abstract Shapes --}}
            <div
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-emerald-500/10 rounded-full blur-[120px] animate-pulse-slow">
            </div>
