{{--
  ...
  Staff Unified Login Page with OTP Support
                ...
                <div x-show="step === 'email'" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="text-center space-y-2 mb-8">
                        ...
                        <p class="text-secondary-500 dark:text-secondary-400">
                            {{ trans('otp.step1_subtitle_staff') }}
                        </p>
                    </div>
                            ...
                            <div class="flex gap-3">
                                ...
                                <div class="text-sm text-accent-700 dark:text-accent-300">
                                    ...
                                    <p class="opacity-90">{{ trans('common.click_to_autofill') }} <span
                                            class="font-mono font-bold group-hover:underline">staff@example.com</span></p>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    <form x-show="userHasPassword" @submit.prevent="attemptLogin" class="space-y-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-end">
                                <a href="{{ route('staff.forgot_password') }}"
        ...
        <div class="hidden lg:flex relative bg-secondary-900 items-center justify-center overflow-hidden">
            ...
            <div class="relative z-10 max-w-lg px-12 text-center animate-fade-in-up delay-200">
                ...
                <h1 class="text-4xl md:text-5xl font-bold text-white mb-8 tracking-tight leading-tight drop-shadow-lg">
                    {!! trans('common.staff_login_block_title') !!}
                </h1>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('staffLoginFlow', (config = {}) => ({
        // State
        step: 'email',
        ...
        async checkEmail() {
            ...
            try {
                const response = await fetch('{{ route('staff.login.check') }}', {
                    method: 'POST',
                    headers: {
        ...
        async attemptLogin() {
            ...
            form.method = 'POST';
            form.action = '{{ route('staff.login.post') }}';
        ...
        async sendOtp() {
            ...
            try {
                const response = await fetch('{{ route('staff.login.otp.send') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                ...
                if (data.success) {
                    window.location.href = '{{ route('staff.login.otp.verify') }}';
                } else {
