...
<div class="min-h-screen bg-gradient-to-br from-stone-50 via-white to-stone-100 dark:from-secondary-950 dark:via-secondary-900 dark:to-secondary-950">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        <div class="absolute top-0 left-1/4 w-96 h-96 bg-primary-500/20 rounded-full blur-3xl animate-float-slow"></div>
        <div class="absolute bottom-1/4 right-1/4 w-80 h-80 bg-blue-500/15 rounded-full blur-3xl animate-float-slow-delayed"></div>
        <div class="absolute top-1/2 right-1/2 w-64 h-64 bg-primary-400/10 rounded-full blur-2xl animate-pulse-glow"></div>
    </div>
    ...
    <div class="relative z-10 w-full max-w-lg mx-auto px-4 py-8 md:py-12">
        @if($member && $card && $reward)
            ...
            {{-- Reward Card Visual --}}
            <div class="mb-6 animate-fade-in-up" style="animation-delay: 135ms;">
                <div class="relative group">
                    {{-- Glow effect --}}
                    <div class="absolute -inset-1 bg-gradient-to-r from-amber-500 via-orange-600 to-amber-500 rounded-3xl opacity-20 group-hover:opacity-30 blur-xl transition-opacity duration-500"></div>
                    <div class="relative">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="space-y-2.5 text-xs mb-4">
                                            <div class="flex items-start justify-between py-1">
                                                ...
                                                <span class="font-semibold text-amber-600 dark:text-amber-400 format-number">{{ $reward->points }}</span>
                                            </div>

                                            <div class="flex items-start justify-between py-1">
                                                <span class="text-secondary-500 dark:text-secondary-400">{{ trans('common.member_balance') }}</span>
                                                <span class="font-semibold {{ $memberBalance >= $reward->points ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }} format-number">
                                                    {{ $memberBalance }}
                                                </span>
                                            </div>
                                        ...
                                        @else
                                            <div class="inline-flex items-center gap-1.5 px-2 py-1 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
                                                <x-ui.icon icon="x-circle" class="w-3 h-3 text-red-600 dark:text-red-400" />
                                                <span class="font-medium text-xs text-red-700 dark:text-red-300">{{ trans('common.member_not_enough_points_for_reward') }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
            ...
            <div class="space-y-6">
                ...
                @if(!$reward)
                    <div class="animate-fade-in-up" style="animation-delay: 100ms;">
                        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200/50 dark:border-red-700/50 rounded-2xl p-8 text-center">
                            <div class="w-16 h-16 mx-auto rounded-2xl bg-red-100 dark:bg-red-900/50 flex items-center justify-center mb-4">
                                <x-ui.icon icon="gift" class="w-8 h-8 text-red-500 dark:text-red-400" />
                            </div>
                            <h3 class="text-lg font-semibold text-red-800 dark:text-red-300">{{ trans('common.reward_not_found') }}</h3>
                        </div>
                    </div>
                @endif
            </div>
